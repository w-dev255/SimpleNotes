...
date=$(date +"%Y-%m-%d-%H_%M_%S")                                                                                       note_name="note_$date.txt"                                                                                              notes=$(realpath notes)

if [[ -e "$notes" ]]; then                                          sleep 1                                             else                                                                echo "$code_name : notes not exists."                       echo "$code_name : Generation in progress..."               mkdir -p notes
fi                                                          
clear
...
while true; do
read -r -p "note: " str                                     if [[ -z "$str" ]]; then
        echo "$code_name : You should add a text..."
else
        echo "$code_name : The string is not empy..."               sleep 1
        break
fi
done
